...
export default function SelectProfilePage() {
  ...
  const loadMembers = useCallback(async (user: FirebaseUser) => {
    setLoading(true);
    try {
      ...
      toast({
        ...
        description: 'No se pudieron cargar los perfiles de la base de datos.',
        variant: 'destructive',
      });
    } finally {
      setLoading(false);
  ...
  const handleAddMember = async (newMemberData: Partial<Member> & { isOwnerProfile?: boolean }) => {
    if (!currentUser) {
    ...
    try {
      ...
      const newMember = await addMember(currentUser.uid, payload, isOwnerProfile);
  ...
  return (
    <>
      ...
      <Dialog open={isAddMemberDialogOpen} onOpenChange={isFirstTime ? () => {} : setIsAddMemberDialogOpen}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>{isFirstTime ? '¡Bienvenido! Crea tu primer perfil' : 'Añadir Nuevo Perfil'}</DialogTitle>
                <DialogDescription>
                  {isFirstTime ? 'Para empezar, crea tu perfil de Estudiante o Adulto. Desde aquí podrás aprender y gestionar tu cuenta familiar.' : 'Crea un nuevo perfil para tu familia.'}
                </DialogDescription>
            </DialogHeader>
            {isFirstTime && (
              <Alert>
                ...
                <AlertDescription>
                  Este perfil te dará acceso a todas las herramientas de aprendizaje. Más tarde, podrás añadir más perfiles para tu familia desde "Gestionar Perfiles".
                </AlertDescription>
              </Alert>
            )}
            <AddMemberForm onAddMember={handleAddMember} isFirstProfile={isFirstTime} />
        </DialogContent>
      </Dialog>
    </>
  );
