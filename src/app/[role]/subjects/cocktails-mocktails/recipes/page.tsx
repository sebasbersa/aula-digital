...
import { ArrowLeft, ChefHat, Sparkles, Loader, Clock, Flame, Info, Save, CheckCircle, X, Trash2, Send, Wand2, Martini } from 'lucide-react';
import type { Recipe, Role, Member, ChatMessage } from '@/lib/types';
import { Textarea } from '@/components/ui/textarea';
import { useToast } from '@/hooks/use-toast';
import Link from 'next/link';
import { useParams } from 'next/navigation';
import { generateFullRecipe } from '@/ai/flows/generate-recipe';
import { generateCocktailIdeas, type GenerateCocktailIdeasOutput } from '@/ai/flows/generate-cocktail-ideas';
        ...
        <div className="space-y-4">
            ...
            {recipe.tip && (
                 <div className="p-3 rounded-md bg-background border border-accent/50 text-sm">
                    <p className="font-semibold flex items-center gap-2"><Info className="w-4 h-4 text-accent" /> Consejo del Bartender IA</p>
                 ...
                 <div className="flex justify-between items-start">
                    <div>
                        <CardTitle className="flex items-center gap-2"><Sparkles className="text-accent" /> {recipe.title}</CardTitle>
                         <CardDescription>
                            Aquí tienes una receta de coctelería creativa con los ingredientes que tienes a mano.
                        </CardDescription>
                    </div>
                        ...
                        <CardContent className="p-4 space-y-4">
                            ...
                            <div className="flex items-center gap-2 pt-2">
                                <Textarea
                                    ...
                                    placeholder="Ej: ¿Puedo usar ron en vez de pisco?"
                                    rows={1}
...
export default function RecipesPage() {
  ...
  const [recipeIdeas, setRecipeIdeas] = useState<GenerateCocktailIdeasOutput['ideas'] | null>(null);
  ...
  const weeklyChallenge = "Esta semana, intenta crear un mocktail usando al menos un ingrediente que nunca hayas mezclado antes.";
  
  const loadRecipes = useCallback(async (profileId: string) => {
    setDataLoading(true);
    try {
        const recipesFromDb = await getRecipes(profileId);
        setSavedRecipes(recipesFromDb.filter(r => r.category === 'Cocktail' || r.category === 'Mocktail'));
    } catch (error) {
  ...
  const handleGenerateIdeas = async () => {
    if (!userInput.trim()) {
        toast({ title: 'Ingredientes faltantes', description: 'Por favor, dime qué ingredientes tienes y qué te gustaría preparar.', variant: 'destructive' });
        return;
    }
    setIsLoading(true);
    setRecipeIdeas(null);
    ...
    try {
        const result = await generateCocktailIdeas({ ingredients: userInput });
  ...
  return (
    ...
    <Dialog open={!!viewingRecipe} onOpenChange={(isOpen) => !isOpen && setViewingRecipe(null)}>
     ...
     <Dialog open={isCategoryDialogOpen} onOpenChange={setIsCategoryDialogOpen}>
        <DialogContent>
            <DialogHeader>
                <DialogTitle>¿Qué tipo de bebida quieres preparar?</DialogTitle>
                <DialogDescription>
    ...
    <div className="container mx-auto py-8 space-y-8">
      <div>
        ...
        <div className="text-center">
             <Martini className="w-12 h-12 mx-auto text-primary" />
             <h1 className="text-4xl font-bold font-headline mt-2">Cocktails y Mocktails</h1>
             <p className="text-muted-foreground mt-2 max-w-2xl mx-auto">
                Usa la IA para crear tragos increíbles con o sin alcohol, y guarda tus favoritos.
            </p>
        </div>
      </div>
      ...
      <Card>
        <CardHeader>
            <CardTitle className="flex items-center gap-2"><Sparkles className="text-accent" /> LIA tu bartender personal</CardTitle>
            <CardDescription>Dime qué licores, jugos o ingredientes tienes, y te daré algunas ideas.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-4">
                ...
                <>
                    <div>
                        ...
                        <Textarea
                            ...
                            placeholder="Ej: Tengo pisco, limón, goma y hielo. Quiero un cóctel clásico."
                            className="mt-1"
                            rows={3}
                        />
                    </div>
