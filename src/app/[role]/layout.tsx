...
export default function AppLayout({
    ...
    const unsubscribe = onAuthStateChanged(auth, async (firebaseUser) => {
      ...
      if (selectedProfileString) {
        ...
        if (selectedProfile.role !== currentRoleInUrl) {
            const matchingProfile = fetchedMembers.find(m => m.role === currentRoleInUrl);
            if (matchingProfile) {
                profileToUse = matchingProfile;
                sessionStorage.setItem('selectedProfile', JSON.stringify(matchingProfile));
            } else {
      ...
      if (profileToUse) {
          setActiveUser({
              ...
              avatarUrl: profileToUse.avatarUrl || `https://placehold.co/100x100.png`,
  ...
  const userRoleDisplay = roleDisplayNames[activeUser.role] || activeUser.role.replace('_', ' ');
    ...
    <FamilyProvider value={{ members, setMembers, currentUser, loading }}>
        ...
        <Sidebar collapsible="none">
            ...
            <div className="flex items-center gap-3">
                ...
                <p className="text-xs text-muted-foreground capitalize">{userDescription}</p>
                </div>
            </div>
            </SidebarFooter>
        </Sidebar>
